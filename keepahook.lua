...
do
	...
	do
		...
		function Library:Watermark(Properties)
			local Watermark = {
				Name = (Properties.Name or Properties.name or "keepaware.club | placeholder");
				AnimateText = nil;
			}
			--
			local Outline = Instance.new("Frame")
			...
			task.spawn(function()
				while task.wait() do
					for i = 1, #"keepaware.club" do
						Watermark.AnimateText = string.sub("keepaware.club", 1, i) .. "";
						Title.Text = Watermark.AnimateText .. " " .. Watermark.Name;
						task.wait(0.4);
					end;

					for i = #"keepaware.club" - 1, 1, -1 do
						Watermark.AnimateText = string.sub("keepaware.club", 1, i) .. "";
						Title.Text = Watermark.AnimateText .. " " .. Watermark.Name;
						task.wait(0.4);
					end;
...
if (not (isfolder("keepaware"))) then
	makefolder("keepaware");
end;
...
local locals = {
	...
	should_keepaware_destroy = false,
	old_ticks = {
...
screen_gui.Name = "https://keepaware.club";
...
screen_gui_2.Name = "https://keepaware.club";
...
do
	...
	do
		...
		instances["local_text"] = utility.instance_new("TextLabel", { 
			Name = "https://keepaware.club",
			Parent = screen_gui_2,
			Text = '<font color="rgb(207, 227, 0)">keepaware</font><font color="rgb(255, 255, 255)">.club</font>',
    ...
    do
		...
		do
			...
			utility.new_connection(run_service.Heartbeat, function()
				...
				if (enabled) then
					...
					local text = custom_text_enabled and '<font color="' .. text_color .. '">' .. features.get_text(custom_text) .. '</font>' or '<font color="' .. text_color .. '">keepaware</font><font color="rgb(255, 255, 255)">.club</font>';
		...
		do
			...
			utility.new_connection(run_service.Heartbeat, function()
				...
				--// keepaware classics
				local classics_enabled = flags["anti_aim_keepaware_classics_enabled"];
				local classics_keybind = flags["anti_aim_keepaware_classics_keybind"];
				local classics_types = flags["anti_aim_keepaware_classics"];
				...
				if ((enabled or classics_enabled) and (keybind or classics_keybind) and (not (classics_keybind and classics_enabled and classics_types == "supercoolboi34 Destroyer") or (classics_keybind and classics_enabled and classics_types == "supercoolboi34 Destroyer"))) then
					...
					local types = {
						...
						["supercoolboi34 Destroyer"] = locals.should_keepaware_destroy and hrp.CFrame + Vector3.new(0 / 0, 1, math.huge) or hrp.CFrame
					};
			...
			task.spawn(function()
				while task.wait(0.1) do
					locals.should_keepaware_destroy = not locals.should_keepaware_destroy;
				end;
			end);
...
do
	if (hookmetamethod) then
		...
		local __index; __index = hookmetamethod(game, "__index", LPH_NO_VIRTUALIZE(function(self, property)
			if (not checkcaller() and ((((flags["anti_aim_c_sync_enabled"] or (flags["anti_aim_keepaware_classics_enabled"] and (flags["anti_aim_keepaware_classics"] == "Destroy Cheaters" or flags["anti_aim_keepaware_classics"] == "supercoolboi34 Destroyer"))) and (flags["anti_aim_c_sync_keybind"] or flags["anti_aim_keepaware_classics_keybind"]))) and property == "CFrame" and self == local_player.Character.HumanoidRootPart)) then
...
do
	...
	do
		...
		local keepaware_classics = ui.tabs["anti_aim"]:Section({Name = "keepaware Classics", Side = "Left", Size = 150});
		...
		--// destroy cheaters
		do
			keepaware_classics:Toggle({Name = "Enabled", Flag = "anti_aim_keepaware_classics_enabled"});
			keepaware_classics:Keybind({Flag = "anti_aim_keepaware_classics_keybind", Name = "Keybind", Default = Enum.KeyCode.Y, Mode = "Toggle"});
			keepaware_classics:List({Name = "Classics", Flag = "anti_aim_keepaware_classics", Options = {"Destroy Cheaters", "supercoolboi34 Destroyer"}, Default = "Destroy Cheaters"});
		end;
	end;
	...
	do --// credits to finobe wtv im way too lazy
		...
		if not isfolder("keepaware/configs") then 
		    makefolder("keepaware/configs");
		end;

		local function update_config_list()
		    local list = {};
		
		    for idx, file in listfiles("keepaware/configs") do
		        local file_name = file:gsub("keepaware/configs\\", ""):gsub(".cfg", ""):gsub("keepaware/configs/", "");
		...
		cfgs:Button({Name = "Create", Callback = function()
		    ...
		    if config_name == "" or isfile("keepaware/configs/" .. config_name .. ".cfg") then
		        return;
		    end;
		    writefile("keepaware/configs/" .. config_name .. ".cfg", Library:GetConfig());
		    update_config_list();
		end});
		...
		cfgs:Button({Name = "Save", Callback = function()
		    ...
		    if selected_config then
		        writefile("keepaware/configs/" .. selected_config .. ".cfg", Library:GetConfig());
		    end;
		end});
		...
		cfgs:Button({Name = "Load", Callback = function()
		    local selected_config = flags.setting_configuration_list;
		    if selected_config then
		        Library:LoadConfig(readfile("keepaware/configs/" .. selected_config .. ".cfg"));
		    end;
		end});
		...
		cfgs:Button({Name = "Delete", Callback = function()
		    local selected_config = flags.setting_configuration_list;
		    if selected_config then
		        delfile("keepaware/configs/" .. selected_config .. ".cfg");
		    end;
		    update_config_list();
		end});
